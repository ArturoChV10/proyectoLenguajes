/* Fernando S�nchez Hidalgo - 2022218688
* Lenguajes de programaci�n S1.2025
*/
...
bool validateUser(string username) {
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            size_t pos = line.find(";"); // encuentra el ; que separa usuario;contrase�a
            if (pos != string::npos) {
...
/* valida que la contrase�a sea de al menos 8 caracteres, contenga una mayuscula y un digito */
bool validatePassword(string pass){
    bool isLong = true;
    if (pass.length() < 8) {
        isLong =  false;
...
/* guarda el registro en users.txt en el formato usuario;contrase�a. La contrase�a es guardada hasheada */
int saveFile(string username, string password){
...
bool loginUser(string username, string password) {
    //recibe un usuario y contrase�a, retorna true si el usuario y contrase�a coinicen, retorna false si no coinciden el usuario y la contrase�a
    ifstream file("users.txt");
    string line, storedUser, storedPass;

    hash<string> hasher;
    string hPassword = to_string(hasher(password)); //hashea la contrase�a ingresada para comparar con la contrase�a guardada
